    ...
    if (yourls_allow_duplicate_longurls() || !($url_exists = yourls_url_exists($url))) {
        ...
        if ($keyword) {
            ...
            if (!yourls_keyword_is_free($keyword)) {
                ...
                $return['message'] = /* //translators: eg "http://someurl/ added to DB" */
                    yourls_s('%s added to database', yourls_trim_long_string($strip_url));
                $return['title'] = $title;
        ...
        $return['message'] = /* //translators: eg "http://someurl/ already exists" */
            yourls_s('%s already exists in database', yourls_trim_long_string($strip_url));
...
function yourls_edit_link($url, $keyword, $newkeyword = '', $title = '')
{
    ...
    if ((!$new_url_already_there || yourls_allow_duplicate_longurls()) && $keyword_is_ok) {
        ...
        if ($update_url) {
            ...
            $return['status'] = 'fail';
            $return['message'] = /* //translators: "Error updating http://someurl/ (Shorturl: http://sho.rt/blah)" */
                yourls_s('Error updating %s (Short URL: %s)', yourls_trim_long_string($strip_url), $keyword);
        }
